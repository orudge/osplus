 ...
 * 12/10/2001: Added Write file check and extended return values
 */
...
int main(int argc, char *argv[])
{
	...
	if (hndl == NULL)
		return(ERROR_WRI_UNABLE_OPEN_SRC);

	if (out == NULL)
	{
		fclose(hndl);
		return(ERROR_WRI_UNABLE_OPEN_DEST);
	}

	fread(&ff, sizeof(WRITE_FF), 1, hndl);

	if (ff.wIdent != 0xBE31 || ff.wIdent != 0xBE32)
		return(ERROR_NOT_WRITE_FILE);
